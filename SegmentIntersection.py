# def add_positive_integers(a: int, b: int) -> int:
#     if not isinstance(a, int) or not isinstance(b, int):
#         raise ValueError("Inputs must be integers")
#     if a < 0 or b < 0:
#         raise ValueError("Inputs must be positive")
#     return a + b + 0

# Andrew Ryan McLinden
# CSCI 332 Spring 2025
# Programming Assignment #class17
# I acknowledge that I have worked on this assignment independently, except where explicitly
# noted and referenced. Any collaboration or use of external resources has been properly cited.
# I am fully aware of the consequences of academic dishonesty and agree to abide by the
# university's academic integrity policy. I understand the importance the consequences of plagiarism.


def find_orientation(p, q, r):

    if p == () or q == () or r == ():
        return -1

    fin = ((q[1]-p[1]) * (r[0]-q[0]) - (r[1] - q[1]) * (q[0] - p[0])) 

    if fin == 0:
        return 0
    elif fin > 0:
        return 1
    else:
        return 2
    
def on_segment(p, q, r):

    if p == () or q == () or r == ():
        return -1
    
    n = find_orientation(p, q, r)
    
    if n == 0 and q[0] >= p[0] and q[0] <= r[0] and q[1] >= p[1] and q[1] <= r[1]:
        return True
    else:
        return False

def do_intersect(l1, l2):
    if l1 == () or l2 == ():
        return -1
    
    orient1 = find_orientation(l1[0], l1[1], l2[0])
    orient2 = find_orientation(l1[0], l1[1], l2[1])
    orient3 = find_orientation(l2[0], l2[1], l1[0])
    orient4 = find_orientation(l2[0], l2[1], l1[1])

    intersect = False
    intersect2 = False
    
    if orient1 != orient2 and orient3 != orient4:
        intersect = True
    
    if orient1 == 0 and on_segment(l1[0], l2[0], l1[1]):
        intersect2 = True
    elif orient2 == 0 and on_segment(l1[0], l2[1], l1[1]):
        intersect2 = True
    elif orient3 == 0 and on_segment(l2[0], l1[0], l2[1]):
        intersect2 = True
    elif orient4 == 0 and on_segment(l2[0], l1[1], l2[1]):
        intersect2 = True

    return intersect ^ intersect2